...
def translate_sublist(
    ...
    Also updates a shared progress bar (thread-safe via 'progress_lock').
    """
    prefix = f"<<{src_lang}>> "
    ...
    for i in range(0, len(sublist), batch_size):
        ...
        # Store each translation in the shared results dict
        for (orig_idx, _, _), trans in zip(batch_chunk, translations):
            results_dict[orig_idx] = trans

        # Update the progress bar with however many items we processed
        with progress_lock:
            progress_bar.update(len(batch_chunk))
...
def main():
    ...
    # 4) Prepare a shared dictionary for results
    translations_dict = {}

    # 5) Prepare a shared tqdm progress bar & lock
    progress_bar = tqdm(total=n_items, desc="Translating", unit="fields")
    progress_lock = threading.Lock()

    # 6) Create two threads, each with a separate tokenizer
    t0 = threading.Thread(
    ...
    # 8) Wait for them to finish
    t0.join()
    t1.join()

    # 9) Close the progress bar
    progress_bar.close()

    # 10) Place translations back
